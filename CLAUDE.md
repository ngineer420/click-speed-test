...
## Thirteen tool pages, one engine (the variant system)

Every search in this niche is a modifier query, so each one gets its own URL.
There are 13 tool pages plus `index.html`, all running the SAME engine. A page
declares itself in the body tag:

```html
<body data-cps-mode="jitter" data-cps-duration="10">
```

- `resolveVariant()` looks the mode up in the `VARIANTS` table (own-property
  check, so `constructor` can't resolve); anything unknown falls back to
  `standard`. `resolveDuration()` validates the duration against
  `ALL_DURATIONS` the same way.
- A variant row declares only: the **input surface** (`left` / `right` /
  `space`), the durations offered, which readouts to show, and a storage
  suffix. **It never carries scoring code.** Every number on every page comes
  out of `analyzeRun()` in the `=== CLICK-ANALYSIS-START/END ===` block — one
  function, fed one array of click timestamps. Adding a page is a row in
  `VARIANTS` plus, at most, a label row in `READOUTS`. If you find yourself
  writing a second scoring function, stop.
- The **six duration pages are all `standard`** with a different
  `data-cps-duration` — a 1-second and a 60-second test are the same
  measurement over a different clock, so they share an engine and a personal
  best exactly as the mode buttons always have. Only a different *technique*
  earns a variant row.
- Every hit funnels through `registerHit()`; the input adapters just decide
  what counts as a hit. Clicks are still counted once, on `pointerdown` (or
  `keydown` on the spacebar page).
- Scores are namespaced by `storageSuffix` (`cbt-best-cps-jitter` and so on).
  `standard` keeps the original unsuffixed keys so existing visitors keep
  their scores.
- Pages ship in both URL forms (`slug.html` + `slug/index.html`, byte
  identical); `sitemap.xml` lists the clean path only. Generated by a
  throwaway script — the repo has no build step, the HTML is the source.
- `assets/js/app.test.js` (`node assets/js/app.test.js`, 27 cases) covers the
...
visitor needs to escape (same bug class as hardware-checkup#7). The rules:
listeners bind to the test surface, **never** to `document`; only the space key
is ever `preventDefault()`-ed, **Tab is left completely alone**; the surface
takes focus explicitly in `startRun()` (it can't hold focus while `disabled`)
and releases it on Escape or focus-out. Auto-repeat is suppressed via
`e.repeat`, so holding the bar counts once. `contextmenu` is suppressed on the
click target only — never globally.
