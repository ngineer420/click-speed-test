...
const P = require("./percentile.js");
const REPO = path.join(__dirname, "../..");

/* Site-specific: every duration model shares one shape, and that shape is the
   one solved from the source's top-tier threshold. */
test("every duration model shares the 10 s shape and reproduces the top-tier threshold", () => {
  for (const m of P.MODELS) {
    const expectedSd = m.mean * Math.sqrt(Math.exp(P.CPS_SIGMA * P.CPS_SIGMA) - 1);
    assert.ok(Math.abs(m.sd - expectedSd) < 0.006, `${m.id}: SD ${m.sd} != mean * shape ${expectedSd.toFixed(3)}`);
  }
  // The published threshold: the top 0.94% on a 10 s test sits at 12.6 CPS.
  const top = P.scoreForPercentile(100 - 0.94, P.CPS_10S);
  assert.ok(Math.abs(top - 12.6) < 0.1, `top 0.94% on 10 s = ${top.toFixed(2)}, expected 12.6`);
  for (const key of ["1", "5", "10", "30", "60", "100clicks"]) {
    assert.ok(P.modelForDuration(key), `no model for duration ${key}`);
  }
  assert.strictEqual(P.modelForDuration("constructor"), null);
  assert.strictEqual(P.modelForDuration("7"), null);
});
